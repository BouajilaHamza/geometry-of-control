...
def slerp_unit(u: torch.Tensor, v: torch.Tensor, t: float) -> torch.Tensor:
    """
    Slerp between unit vectors u and v along last dim.
    u,v: [..., d] assumed unit norm.
    """
...
def apply_normalized_steer(h_last: torch.Tensor, v: torch.Tensor, alpha: float, mode: str) -> torch.Tensor:
    """
    h_last: [batch, 1, d] current position hidden state
    v:      [1, 1, d] steering vector (unit norm)
    """
    if mode == "add":
    ...
    if mode == "slerp":
        # Interpret alpha as interpolation fraction (clamped to [0,1]).
        u = h0 / n0
        vv = v.expand_as(h0)
...
class SafetySteeringHook:
    ...
    def enable(self, strength: float, *, mode: str):
        ...
        def inject_hook(_m, _inp, out):
            ...
            hs2 = hs.clone()
            hs2[:, -1:, :] = apply_normalized_steer(hs2[:, -1:, :], self.steering_vector, strength, self.mode)
...
def main() -> None:
    ...
    ap.add_argument("--steering-mode", choices=["add", "renorm", "slerp"], default="renorm")
    ap.add_argument("--steering-strength-final", type=float, default=8.0)
    ap.add_argument("--sweep", default="2,5,10,20", help="Comma-separated layer strengths to try (defense ON).")
    ...
    print(f"Steering mode: {args.steering_mode}")
    print(f"Injection layer: {args.layer} | sweep(layerα)={args.sweep} | finalα={args.steering_strength_final}")
    ...
    gen_kwargs = dict(
        ...
        logits_processor=LogitsProcessorList([attacker, entropy_logger]),
    )
    ...
    results.append(
        RunResult(
            ...
            avg_entropy=(statistics.mean(entropy_logger.entropies) if entropy_logger.entropies else None),
            max_entropy=(max(entropy_logger.entropies) if entropy_logger.entropies else None),
            text=safe1,
        )
    )
    ...
    for a_layer in sweep:
        ...
        layer_def.hook_ms.clear()
        final_def.hook_ms.clear()
        entropy_logger.entropies.clear()
        layer_def.enable(a_layer, mode=args.steering_mode)
        final_def.enable(args.steering_strength_final, mode=args.steering_mode)
    ...
    for r in results:
        ...
        print(
            ...
            f"{hook} | refused={r.refused} attempted_instructions={r.attempted_instructions} "
            f"avg_entropy={None if r.avg_entropy is None else round(r.avg_entropy,2)} "
            f"max_entropy={None if r.max_entropy is None else round(r.max_entropy,2)} "
            f"compliance_preface={r.compliance_preface} redacted={r.redacted}"
        )
    print()
